    ...
    public partial class frmExaminarProduccion_Proveedor : Form
    {
        ...
        private void TBBuscar_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    this.DGResultados.DataSource = fProduccion_Proveedor.Examinar_PorCodigo(this.TBBuscar.Text);
                    this.DGResultados.Columns[0].Visible = false;
                    lblTotal.Text = "Datos Registrados: " + Convert.ToString(DGResultados.Rows.Count);
                }
                else if (CBBuscar.SelectedIndex == 2)
                {
                    this.DGResultados.DataSource = fProduccion_Proveedor.Examinar_PorIdentificacion(this.TBBuscar.Text);
                    this.DGResultados.Columns[0].Visible = false;
                    lblTotal.Text = "Datos Registrados: " + Convert.ToString(DGResultados.Rows.Count);
                }
                else if (CBBuscar.SelectedIndex == 3)
                {
                    this.DGResultados.DataSource = fProduccion_Proveedor.Examinar_PorNombre(this.TBBuscar.Text);
                    this.DGResultados.Columns[0].Visible = false;
                    lblTotal.Text = "Datos Registrados: " + Convert.ToString(DGResultados.Rows.Count);
                }
            }
